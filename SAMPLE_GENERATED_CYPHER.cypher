...
// Auto-generated by batch_test_folders.py
...
// Much more efficient than per-individual statements
//
...
// ============================================================================
// VERIFICATION - Run these queries to confirm blocks were applied
// ============================================================================

// Check total blocked edges
MATCH (r:in_register_with)
WHERE r.block = ['No expression in region']
RETURN COUNT(r) as blocked_count, COLLECT(DISTINCT r.folder[0]) as folders;

// List all blocked registrations with details
MATCH (n:Individual)-[r:in_register_with]->(tc:Template)
WHERE r.block = ['No expression in region']
RETURN 
    n.short_form as individual,
    n.label as individual_label,
    r.folder[0] as folder,
    tc.label as template,
    r.block as blocked_reason
ORDER BY tc.short_form, r.folder[0];

// Verify Brain blocks
MATCH (r:in_register_with)->(tc:Template {short_form: 'VFB_00101567'})
WHERE r.block = ['No expression in region']
RETURN COUNT(r) as brain_blocked;

// Verify VNC blocks  
MATCH (r:in_register_with)->(tc:Template {short_form: 'VFB_00200000'})
WHERE r.block = ['No expression in region']
RETURN COUNT(r) as vnc_blocked;
